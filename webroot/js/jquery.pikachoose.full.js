...
(function($) {
     ...
     */
 	var defaults = {
		autoPlay: true,
		speed: 5000,
		text: { play: "", stop: "", previous: "back", next: "next" },
		transition:[-1],
		showCaption: true,
		IESafe: false,
		showTooltips: false,
    ...
    $pc.fn.extend({
        ...
        build: function() {
       	...
       	//create the structure for pikachoose
			...
			if(this.options.useThumbsNav){
				this.list.addClass('pika-thumbs'); //Qwien Add
			}
			...
			// add opacity to navigation Q
			if(this.options.navOpacity){
				var opacityVal = this.options.navOpacityVal;
				$('.jcarousel-item').fadeTo(250,this.options.navOpacityVal);
				
				$('.jcarousel-item').hover(function () {
					$('.jcarousel-item').stop().fadeTo(250, 1);
				}, function () {
					$('.jcarousel-item').stop().fadeTo(250,opacityVal);
				});
			}
